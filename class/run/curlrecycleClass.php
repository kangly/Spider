...
//抓取东莞市加工贸易废料网上交易平台
class curlrecycleClass extends baseClass
{
    function run()
    {
        $this->curl_recycle();
    ...
    protected function curl_recycle()
    {
        $this->start_task(28);
        ...
        for($i=1;$i>0;$i++)
        {
            ...
            if($i>1)
            {
                $details = QueryList::run('Request', [
                    ...
                    'Host' => 'www.dg-recycle.com',
                    'Origin' => 'http://www.dg-recycle.com',
                    'X-Requested-With' => 'XMLHttpRequest',
                    'Cache-Control' => 'no-cache',
                    'X-MicrosoftAjax' => 'Delta=true',
                    'Content-Type' => 'application/x-www-form-urlencoded; charset=UTF-8',
                    'Referer' => 'http://www.dg-recycle.com/Trade_Announcement/AnnouncementList.aspx?mid=1107131&fid=0',
                    'params' => [
            ...
            foreach($list_data as $v)
            {
                ...
                $view_url = 'http://www.dg-recycle.com/Trade_Announcement/AnonymousAnnounceDetail.aspx?id='.$v['id'];
                $is_change = $this->dbm->select('collect_data', ['id'], ['url'=>$view_url,'pid'=>28]);
                if(empty($is_change))
                {
                    sleep(6);

                    $content_url = 'http://www.dg-recycle.com/Service/DisplayAnnouncementContent.ashx?contents='.$v['id'];
                    ...
                    $img_array = QueryList::Query($content, array(
                        'url' => array('a','href','',function($content){
                            return 'http://www.dg-recycle.com'.str_replace('../..','',$content);
                        }),
                        'name' => array('a','text')
                    ),'','UTF-8','UTF-8',true)->data;
                    ...
                    foreach($img_array as $fk=>$f){
                        if(strpos($f['url'],'type=download') !== false){
                            ...
                            $local_file = $img_path.'/recycle_'.$fk.time().'.'.$ext;
                    ...
                    $file_array = QueryList::Query($view_url, array(
                        'url' => array('#ctl00_ContentPlaceHolder1_txtContract_Sample a','href','',function($content){
                            return 'http://www.dg-recycle.com'.$content;
                        }),
